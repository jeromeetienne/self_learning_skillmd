...
- `test_cases.json`: 20 changes to stage, each with its user message, its issue number, and whether the change is a fix.
...
- The `user_message` of a test case never names a rule. It gives only the facts: the issue, and whether the change fixes it.
