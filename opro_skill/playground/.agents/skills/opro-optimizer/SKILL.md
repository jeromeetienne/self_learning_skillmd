...
- **Stop the rounds** when two rounds in a row give no version with a score above the best score, or when the user set a number of harness runs and the loop reached it. Then **go to step 5 and finish the run**: the `final_check` group, `finish-run`, and the report. Say in the report that the loop stopped before the last round, and why. Ask the user for more rounds only after the report, and never end your turn with a question and an unfinished run, because the user may read your answer long after the run, or never.
