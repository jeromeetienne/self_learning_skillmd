...
- `harness_types.ts`, `harness_command.ts`, `harness_run.ts`, `git_command.ts`, `concurrency.ts`: the harnesses and their fixed models, the command line of a harness, a harness run until the end of its turn, git with a fixed author, and a map with a maximum concurrency.
...
- Nothing here imports from `test_skills/`, or from any other folder of the repository. The package must work after it is published on its own.
...
## Background
- The rules as data give the same verdict as the deleted code that they replaced: 3842 rule checks with no mismatch, in milestone 1 of [issue #4](https://github.com/jeromeetienne/skillmd_opro/issues/4).
- A nested harness cannot change a file: Codex runs with `--sandbox read-only` and ignores `~/.codex/config.toml` with `--ignore-user-config`, and Claude Code runs in `--print` mode with only the allowed tools and ignores the user settings with `--setting-sources project,local`. The standard input of a harness is closed, because Codex waits for more input on an open standard input.
